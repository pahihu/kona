...
#ifdef DEBUG
I sizeofSym(S s){I nb=(strlen(s)+1+sizeof(I)-1)/sizeof(I);R nb*sizeof(I);}
void tf(N n){if(!n)R;DO(2,tf(n->c[i]));if(n->k)repool(n->k-NSLOTS*sizeof(I),lsz(sizeofSym(n->k)));repool(n,lszNode);} //tree free
V krec[1000000];
